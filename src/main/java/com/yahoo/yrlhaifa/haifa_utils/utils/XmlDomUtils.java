...
/**
 * Utilities for the DOM API of XML I/O (package org.w3c.dom in J2SE standard library). (Copied from
 * Excitement-open-platform code)
 * 
 * @author Asher Stern
 * @since Oct 3, 2012
 *
...
public class XmlDomUtils {
    ...
    public static class XmlDomUtilitiesException extends Exception {
    ...
    /**
     * Returns an element which is a direct child of the given element, and has the name "childName".
     * 
     ...
     * Returns an element which is a direct child of the given element, and has the name "childName".
     * <P>
     * If <code>optional</code> is false, then if the required element does not exist, the function will throw an
     * exception.
     * 
     * @param element A given element (we need its child)
     * @param childName The name of the child-element that we are looking for.
     * @param optional Indicates whether this element must exist, and if it does not exist it is an error and an
     *        exception will be thrown (optional==false), or not (optional==true).
     * @return The child-element, which is a direct child of the given element, and named "<code>childName</code>".
     * @throws TreeXmlException
     */
    public static Element getChildElement(Element element, String childName, boolean optional)
            ...
            if (node instanceof Element) {
                ...
                if (elementOfNode.getTagName().equals(childName)) {
                    if (childElement != null)
                        throw new XmlDomUtilitiesException("More than one child: " + childName);
    ...
    /**
     * Given an XML element, this method returns nested elements which are direct children of the given element - but
     * only those that have the element-name "childrenName" (the second parameter).
     * 
     * @param element
     * @param childrenName
     * @return
    ...
    /**
     * Returns the text of an XML element, as String. It usually looks like (please look at the following lines in an
     * HTML viewer)<BR>
     * &lt;element-name&gt;<BR>
     * some text<BR>
     * &lt;/element-name&gt;<BR>
     ...
     * <P>
     * The boolean parameter (textMustExist) indicates whether it is legal that no text exists for the element. If it is
     * legal, and there is no text, then the return value is null. If it is not legal, and there is no text, then an
     * XmlDomUtilitiesException exception is thrown.
     * 
     * @param element the element which we want to get its text.
     * @param textMustExist <tt>true</tt> if it is not legal that the given element has no text.
     * 
     * @return the text of the element. Null if the text does not exist, and textMustExist is <tt>false</tt>.
     * @throws XmlDomUtilitiesException
     */
    public static String getTextOfElement(Element element, boolean textMustExist) throws XmlDomUtilitiesException {
        ...
        for (int index = 0; index < nodeList.getLength(); index++) {
            ...
            if (node instanceof Text) {
                ...
                if (str.length() > 0) {
                    if (foundString != null)
                        throw new XmlDomUtilitiesException("Unexpected XML contents. Element has more than one text.");
                    foundString = str;
                }
            }
        }
        if (textMustExist) {
            if (null == foundString)
                throw new XmlDomUtilitiesException("Unexpected XML contents. Text was not found for element: "
                                + element.toString());
        }
        return foundString;
    }
    ...
    /**
     * Writes the given document into a file.
     * 
     * @param document an XML-DOM document
     * @param file the file into which that document will be written. If the file is not empty, its content will be
     *        overridden.
     * @throws XmlDomUtilitiesException
     */
